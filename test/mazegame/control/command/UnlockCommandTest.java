...
public class UnlockCommandTest {
	...
	void whenPlayerHasBannerAtValidCastleEntrance_shouldSuccessfullyUnlock() {
		...
		assertEquals("You use the banner to unlock the path. The way to Gregor's castle is now open!", 
				response.getMessage(),
				"Should successfully unlock Town Square southwest exit with banner.");
		assertFalse(townSquareExit.isLocked(),
				"Town Square southwest exit should be unlocked after using banner.");
		...
		assertEquals("You use the banner to unlock the path. The way to Gregor's castle is now open!", 
				response.getMessage(),
				"Should successfully unlock Crystal Cave west exit with banner.");
		assertFalse(crystalCaveExit.isLocked(),
				"Crystal Cave west exit should be unlocked after using banner.");
		...
		assertTrue(crystalCaveExit.isLocked(),
				"Exit should remain locked when player doesn't have banner.");
	}
}