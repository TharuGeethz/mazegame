...
/**
 * Command to sell an item at a blacksmith. Pays 80% of the item's listed value;
 * blocked in combat or outside shops.
 */
public class SellCommand implements Command {
	// If sold player receives the value listed on the weapon or armour table less
	// 20%
	@Override
	public CommandResponse execute(ParsedInput userInput, Player currentPlayer) {
		...
		if (currentPlayer.getWearingItems().containsKey(itemNameLower)) {
			return new CommandResponse("You cannot sell " + itemNameLower + " while it's equipped. Remove it first.");
		}
		...
		return new CommandResponse("You sold " + soldItem.getLabel() + " for " + sellPrice + " gold pieces (80% of "
				+ itemValue + " gold value). You now have " + currentPlayer.getInventory().getGold().getTotal()
				+ " gold pieces.");
	}
}
