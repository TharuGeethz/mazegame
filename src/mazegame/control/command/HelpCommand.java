...
/**
 * Builds a contextual help message based on the player's situation such as
 * being in a shop, in combat or exploring.
 */
public class HelpCommand implements Command {
	@Override
	public CommandResponse execute(ParsedInput userInput, Player currentPlayer) {

		StringBuilder helpText = new StringBuilder(); // accumulator for help lines
		helpText.append("====== MAZE GAME HELP ======\n\n");

		// detect context
		boolean inShop = currentPlayer.getCurrentLocation() instanceof Blacksmith; // shop check
		boolean inCombat = currentPlayer.getCurrentLocation().getNpcCollection().hasHostileNPCs(); // combat check
		...
		if (inShop) {
			...
			helpText.append("  move <direction>     - Move in a direction (north, south, east, west)\n\n");

		} else if (inCombat) {
			helpText.append("You are now in Combat mode!. Available commands:\n\n");
		...
		return new CommandResponse(helpText.toString()); // return final help text
	}
}
